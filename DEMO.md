...
- 你的 CSV 列名必须严格为：`t`, `vdiff`（单位分别为 s / V）。
...
### 报错：`CSV must contain column "t"` 或 `"vdiff"`
- 说明 CSV 列名不对，改成严格的 `t, vdiff`。
...
现在已拆成两个子模块，顶层 `qprbs13_pipeline` 会自动串联：

- `qprbs13_resample_module(cfg)`：完成 CSV -> 对齐扫描 -> `y_avg` / `Y`
- `qprbs13_symbol_module(Y, cfg)`：完成 `Y` -> `sym_code` / `ES1` / `ES2` / `RLM` / `x`

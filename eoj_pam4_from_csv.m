...
%   we hold previous estimate (sample-and-hold behavior).
    ...
    all_evt = collect_all_crossing_events_from_symbols(t_uniform, v_uniform, sym, cfg.M, th01, th12, th23, V0, V1, V2, V3, ui_start_for_use);
    ...
    for i = 1:12
        ...
        [a_nm, b_nm, ok_nm] = parse_transition_name_pair(trans_def(i).name);
        if ok_nm
            th = threshold_from_symbol_pair(a_nm, b_nm, V0, V1, V2, V3);
        else
            th = get_threshold(trans_def(i).thr_type, th01, th12, th23, V0, V1, V2, V3);
        end
    ...
    % Step 7: EOJ measurement without CRU filtering (requested behavior).
    % Keep raw crossing timeline directly for EOJ statistics.
    tie_LF_ui = zeros(N_UI, 1);
    for i = 1:12
        trans(i).tcross_cru_abs = trans(i).tcross_abs;
    end
    % Step 8: choose 3 adjacent repeats and reference transition.
    % Reference period estimate uses repeatA/repeatB on reference transition.
    % EOJ_i uses repeatB/repeatC on every transition.
    [ref_i, repA, repB, repC] = choose_eoj_repeat_triplet_and_reference(trans, cfg.ref_transition, use_repeat_start, cfg.verbose);

    T3 = mean_or_nan(trans(ref_i).tcross_cru_abs(trans(ref_i).repeat_id == repA));
    T4 = mean_or_nan(trans(ref_i).tcross_cru_abs(trans(ref_i).repeat_id == repB));

    if isnan(T3) || isnan(T4)
        error('Reference transition has no valid data in selected repeatA/repeatB.');
    end
    Tpat_est = T4 - T3;

    % Step 9: EOJ_i on another adjacent pair (repeatB/repeatC)
    T1 = nan(1, 12);
    T2 = nan(1, 12);
    EOJ_i = nan(1, 12);
    for i = 1:12
        T1(i) = mean_or_nan(trans(i).tcross_cru_abs(trans(i).repeat_id == repB));
        T2(i) = mean_or_nan(trans(i).tcross_cru_abs(trans(i).repeat_id == repC));
        if isnan(T1(i)) || isnan(T2(i))
            warning('Transition %d (%s) missing in repeat%d/repeat%d. EOJ_i set to NaN.', i, trans(i).name, repB, repC);
            EOJ_i(i) = nan;
        else
    ...
    out.repeat_pair_used = [repB, repC];
    out.ref_repeat_pair_used = [repA, repB];
    out.repeat_triplet_used = [repA, repB, repC];

    out.V0 = V0; out.V1 = V1; out.V2 = V2; out.V3 = V3;
    out.symbol_mean_voltage = [V0, V1, V2, V3];
    out.centers_sorted = centers_sorted;
    out.th01 = th01; out.th02 = th02; out.th03 = th03;
    out.th12 = th12; out.th13 = th13; out.th23 = th23;
    out.crossing_threshold = struct('th01', th01, 'th02', th02, 'th03', th03, ...
        'th12', th12, 'th13', th13, 'th23', th23);
    ...
    out.all_crossing_threshold_times = struct( ...
        ...
        'th02', all_evt.tcross_abs(all_evt.th_id == 2), ...
        'th03', all_evt.tcross_abs(all_evt.th_id == 3), ...
        'th12', all_evt.tcross_abs(all_evt.th_id == 4), ...
        'th13', all_evt.tcross_abs(all_evt.th_id == 5), ...
        'th23', all_evt.tcross_abs(all_evt.th_id == 6));
    ...
    % Step 11: plots
    if cfg.do_plot
        make_plots(y, [V0 V1 V2 V3], [th01 th12 th23], out, trans, repB, repC);
    end
    ...
    if cfg.verbose
        ...
        fprintf('[EOJ] Crossing thresholds [th01 th02 th03 th12 th13 th23] = [%.6g %.6g %.6g %.6g %.6g %.6g] V\n', ...
            out.th01, out.th02, out.th03, out.th12, out.th13, out.th23);
...
function all_evt = collect_all_crossing_events_from_symbols(t, v, sym, M, th01, th12, th23, V0, V1, V2, V3, ui_start)
    tc = [];
    ui = [];
    th_id = [];
    th_val = [];
    ...
    for n = n0:n1
        ...
        th = threshold_from_symbol_pair(a, b, V0, V1, V2, V3);
        dir = ternary(b > a, 'rise', 'fall');

        [k0, k1] = boundary_half_ui_range(n, M, numel(v));
        ...
        th_id(end+1,1) = threshold_id_from_pair(a, b); %#ok<AGROW>
...
function th = get_threshold(thr_type, th01, th12, th23, V0, V1, V2, V3)
...
function th = threshold_from_symbol_pair(a, b, V0, V1, V2, V3)
    levels = [V0, V1, V2, V3];
    ai = a + 1;
    bi = b + 1;
    if ai < 1 || ai > 4 || bi < 1 || bi > 4
        error('Invalid symbol pair %d->%d for threshold computation.', a, b);
    end
    th = (levels(ai) + levels(bi)) / 2;
end

function id = threshold_id_from_pair(a, b)
    lo = min(a, b);
    hi = max(a, b);
    if lo == 0 && hi == 1
        id = 1; % th01
    ...
    elseif lo == 2 && hi == 3
        ...
        error('Unknown threshold pair from symbols %d/%d.', a, b);
    end
end

function [k0, k1] = boundary_half_ui_range(n, M, Nv)
    % Restrict search to right half of UI n and left half of UI n+1.
    % UI n spans [(n-1)M+1, nM], UI n+1 spans [nM+1, (n+1)M].
...
function [ref_i, repA, repB, repC] = choose_eoj_repeat_triplet_and_reference(trans, ref_cfg, rep_start, verbose)
    % Need three adjacent repeats: (repA, repB, repC).
    % Tpat_est uses reference transition on repA/repB.
    % EOJ_i uses all transitions on repB/repC.

    best = -inf;
    found = false;
    repA = nan; repB = nan; repC = nan; ref_i = 1;

    if ~isempty(ref_cfg)
        cand_ref = ref_cfg;
        [repA, repB, repC, ok, score] = best_triplet_for_reference(trans, cand_ref, rep_start);
        if ok
            ref_i = cand_ref;
            if verbose && repA ~= rep_start
                warning('ref_transition=%d not available on repeat%d/repeat%d/repeat%d; fallback to repeat%d/repeat%d/repeat%d.', ...
                    ref_i, rep_start, rep_start+1, rep_start+2, repA, repB, repC);
            end
            return;
        end
        error('Specified ref_transition=%d has no valid 3-adjacent-repeat triplet.', ref_cfg);
    end

    for i = 1:numel(trans)
        [a,b,c,ok,score] = best_triplet_for_reference(trans, i, rep_start);
        if ok && score > best
            best = score;
            ref_i = i;
            repA = a; repB = b; repC = c;
            found = true;
        end
    end

    if ~found
        error('No valid 3-adjacent-repeat triplet found for EOJ computation.');
    end
end

function [repA, repB, repC, ok, score] = best_triplet_for_reference(trans, ref_i, rep_start)
    repA = nan; repB = nan; repC = nan; ok = false; score = -inf;

    max_rep = -inf;
    for i = 1:numel(trans)
        if ~isempty(trans(i).repeat_id)
            max_rep = max(max_rep, max(trans(i).repeat_id));
        end
    end
    if ~isfinite(max_rep)
        return;
    ...
    for a = rep_start:(max_rep-2)
        b = a + 1; c = a + 2;
        c_ref_a = sum(trans(ref_i).repeat_id == a);
        c_ref_b = sum(trans(ref_i).repeat_id == b);
        if c_ref_a == 0 || c_ref_b == 0
            continue;
        end

        % Score prefers: strong reference on A/B + rich events on B/C across all transitions.
        bc_total = 0;
        for k = 1:numel(trans)
            bc_total = bc_total + sum(trans(k).repeat_id == b) + sum(trans(k).repeat_id == c);
        end
        sc = 1000 * (c_ref_a + c_ref_b) + bc_total;
        if sc > score
            score = sc;
            repA = a; repB = b; repC = c;
            ok = true;
...
function files = export_eoj_csv_outputs(out, cfg, csv_file, N_UI)
    ...
    for k = 1:numel(th_name)
        ...
        elseif out.all_crossings.th_id(k) == 2
            th_name(k) = "th02";
        elseif out.all_crossings.th_id(k) == 3
            th_name(k) = "th03";
        elseif out.all_crossings.th_id(k) == 4
            th_name(k) = "th12";
        elseif out.all_crossings.th_id(k) == 5
            th_name(k) = "th13";
        else
            th_name(k) = "th23";
        end
    ...
    T_sum = table(out.EOJ_sec, out.EOJ_ps, out.EOJ_UI, out.Tpat_est, ...
        ...
        out.T3, out.T4, out.th01, out.th02, out.th03, out.th12, out.th13, out.th23, ...
        'VariableNames', {'EOJ_sec','EOJ_ps','EOJ_UI','Tpat_est_s', ...
        'ref_transition','repeat0','repeat1','T3_s','T4_s', ...
        'th01_V','th02_V','th03_V','th12_V','th13_V','th23_V'});

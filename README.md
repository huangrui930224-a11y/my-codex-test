...
- 根据用户要求，重采样起点从第一个 crossing time 开始。
- 在原始波形上以 `mean(v_raw)` 为起始阈值，找首个过阈值区间。
- 通过线性插值得到 `t_start_resample`。
- 若未找到 crossing，回退到 `t_raw(1)` 并告警。
...
- 取每个 UI 的中心点（`m = round(M/2)`）作为 `y_center`。

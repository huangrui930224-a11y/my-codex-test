...
- `ui_sampling_columns`（每个 UI 导出 CSV 的列名）
- `use_cru`（本次分析是否启用 CRU）
...
### Step 1.5 估计重采样起点（首个 AB 阈值过点）

- 重采样起点定义为“**存在 A->B 跳变的第一个过阈值采样点**”。
- 先在原始波形上做 4 电平聚类，得到粗符号 `A/B` 与电平均值。
- 由电平均值计算 6 组阈值：`th01/th02/th03/th12/th13/th23`。
- 按首个 `A->B` 跳变选择对应阈值，并寻找第一个实际过阈值采样点。
- 将该样本时间设为 `t_start_resample`（不做时间插值）。
...
- 在“前一 UI 右半边 + 当前 UI 左半边”拼接窗口中搜索 crossing（抑制远离边界的伪 crossing）。
- 根据跳变类型使用对应阈值（`01/02/03/12/13/23`）。

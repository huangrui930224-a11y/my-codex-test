...
代码内部仍保留 `th01/th12/th23` 基准，并在 crossing 时按符号对直接计算对应中点阈值。
...
1. **全 crossing 提取**（给 CRU 用）：
   ...
   - 再按该跳变对应阈值（`th01/th02/th03/th12/th13/th23`）与方向做 crossing 插值
...
不是。当前实现是**过阈值点**（`th01/th02/th03/th12/th13/th23`）并线性插值。

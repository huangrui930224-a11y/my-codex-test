function [y, Y, info] = resample_qprbs13_cei_step1(t, v_tx, M, t_cycle1_ui1)
%RESAMPLE_QPRBS13_CEI_STEP1 Step-1 resampling per CEI workflow.
%   [y, Y, info] = resample_qprbs13_cei_step1(t, v_tx, M, t_cycle1_ui1)
%
% Inputs
%   t    : time vector (s)
%   v_tx : TX analog output waveform (V)
%   M    : samples per UI (default 64)
%   t_cycle1_ui1 : time of cycle-1 / UI-1 boundary (optional)
%
% Outputs
...
%   N1           : 20*N analyzed symbols (20 cycles)
    ...
    if nargin < 4 || isempty(t_cycle1_ui1)
        % Default assumption (same as previous behavior):
        % input waveform starts at cycle-1 / UI-1.
        t_cycle1_ui1 = t(1);
        anchor_source = 'assumed_from_t1';
    else
        anchor_source = 'user_provided';
    end

    % ---- Step 1: strict time window ----
    % Start from first UI of cycle #2 and end at last UI of cycle #21.
    % Each cycle has N UIs.
    t_start = t_cycle1_ui1 + N * UI;
    t_end = t_start + N1 * UI;
    ...
    info.N1 = N1;
    info.M = M;
    info.t_cycle1_ui1 = t_cycle1_ui1;
    info.anchor_source = anchor_source;
